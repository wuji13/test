...
def buy_stock(context,stock ,data_dict):
    max_position = 0.5  #最大买入仓位 
    stock_buy_num = 2 #最多买入5支股票
...
def sell_stock(context,stock_list,data_dict):
    for stock in list(context.portfolio.positions.keys()):
        close = get_history(5,'1d','close')[stock].values
        ...
        if yesterday_close<ma5.avgs[0]:
           order_target_value(stock,0)  #表中则全如果不在股票列部卖出
...
def choose_stock_finance():
    dataframe = get_fundamentals(
     query(
         fundamentals.equity_valuation_indicator.pe_ratio   #总市值
     ).filter(
         fundamentals.equity_valuation_indicator.pe_ratio < 15  #总市值小说200亿  
...
def ma_bull_stock(stock):
    ...
    if ma5.avgs[0]<ma5.avgs[1]<ma5.avgs[2] and\
        ma10.avgs[0]<ma10.avgs[1]<ma10.avgs[2] and\
        ma20.avgs[0]<ma20.avgs[1]<ma20.avgs[2]:
        return True
    else:
        return False
